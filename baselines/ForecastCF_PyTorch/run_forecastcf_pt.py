"""
Runner CLI pour ForecastCF-PyTorch.

Usage:
    python baselines/ForecastCF_PyTorch/run_forecastcf_pt.py --dataset etth1 --model gru --seeds 1 9 30
"""
...
def run_forecastcf_pt_single_seed(dataset, model, seed, device, n_batches=20):
    """
    Exécute ForecastCF-PyTorch pour un (dataset, model, seed) donné.
    ...
    print(f"\n[ForecastCF-PyTorch] Dataset={dataset}, Model={model}, Seed={seed}")
    ...
    forecastcf = ForecastCFPyTorch(
        ...
        max_iter=150,
        lr=1e-3,
        pred_margin_weight=0.8  # 80% validity, 20% proximity
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run ForecastCF-PyTorch baseline")
    ...
    parser.add_argument("--output_dir", type=str, default="baselines/ForecastCF_PyTorch/results")
    ...
    for seed in args.seeds:
        result = run_forecastcf_pt_single_seed(args.dataset, args.model, seed, device, args.n_batches)
    ...
    print(f"\n{'='*60}")
    print(f"ForecastCF-PyTorch — {args.dataset}/{args.model}")
    print(f"{'='*60}")
