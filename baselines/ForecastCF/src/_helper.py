...
def load_dataset(dataset_name, data_path):
    if dataset_name == "cif2016":
        ...
        # ETTh1: single univariate series (OT column).
        # Reshape to (1, T) so DataLoader treats it as one long series.
        raw = pd.read_csv(data_path + "ETTh1.csv")
        ot  = raw["OT"].values.astype("float")   # shape (T,)
        df  = pd.DataFrame(ot.reshape(1, -1))     # shape (1, T)
    else:
