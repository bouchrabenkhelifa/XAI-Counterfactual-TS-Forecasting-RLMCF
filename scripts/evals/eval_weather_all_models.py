"""
Eval Weather — 5 modèles
Usage: python scripts/evals/eval_weather_all_models.py
"""
...
OUTPUT_DIR = "assets/results/weather_summary"

MODELS = [
    ("iTransformer", "assets/configs/models/weather_dataset/RL/config_itransformer.json",
     "assets/configs/models/weather_dataset/forecasters/itransformer/weather_96_48_S.json"),
    ("PatchTST",     "assets/configs/models/weather_dataset/RL/config_patchtst.json",
     "assets/configs/models/weather_dataset/forecasters/patchtst/weather_96_48_S.json"),
    ("TimesNet",     "assets/configs/models/weather_dataset/RL/config_timesnet.json",
     "assets/configs/models/weather_dataset/forecasters/timesnet/weather_96_48_S.json"),
    ("GRU",          "assets/configs/models/weather_dataset/RL/config_gru.json",
     "assets/configs/models/weather_dataset/forecasters/gru/weather_96_48_S.json"),
    ("DLinear",      "assets/configs/models/weather_dataset/RL/config_dlinear.json",
...
def eval_one(label, rl_cfg, f_cfg, ae_cfg, device):
    print(f"\n{'='*60}\nEvaluating: {label}\n{'='*60}")
    cfg_rl = load_config(rl_cfg)
    cfg_f  = load_config(f_cfg)
    cfg_ae = load_config(ae_cfg)
    trainer = RLMaskTrainer(cfg_f, cfg_ae, cfg_rl, device)
    ckpt_path = os.path.join(cfg_rl.checkpoint_dir_lp, f"{cfg_rl.name}_agent_best.pt")
...
def print_table(results):
    ...
    print(f"\n{'='*60}\n  Weather — Full Evaluation (5 models)\n{'='*60}")
    print(header)
    print("-" * (14 + col_w * len(TABLE_METRICS) + 2))
...
def plot_radar(results, out_dir):
    metrics_radar = [("validity_ratio","Validity"),("stepwise_auc","AUC"),
                     ("compactness","Compact."),("temporal_consistency","T-Cons.")]
    metrics_inv   = [("proximity_l2","Prox.(inv)"),("plausibility_ensemble","Plaus.(inv)")]
    ...
    for (label, summary), color in zip(results.items(), colors):
        ...
        vals += [float(np.clip(1.0-summary.get(k,{}).get("mean",0.0),0,1)) for k,_ in metrics_inv]
        vals += vals[:1]
        ax.plot(angles, vals, "o-", lw=2, color=color, label=label)
        ax.fill(angles, vals, alpha=0.08, color=color)
    ax.set_xticks(angles[:-1]); ax.set_xticklabels(labels, fontsize=11)
    ax.set_ylim(0,1)
    ax.set_title("Weather — RL-MCF across 5 forecasters", fontsize=13, fontweight="bold", pad=20)
    ax.legend(loc="upper right", bbox_to_anchor=(1.3,1.1), fontsize=10)
    fig.tight_layout()
    p = os.path.join(out_dir, "weather_radar_5models.png")
...
def plot_barplot(results, out_dir):
    ...
    for i, (label, color) in enumerate(zip(models, colors)):
        ...
        ax.bar(x+i*w-(len(models)-1)*w/2, vals, w, yerr=stds, capsize=3,
               color=color, alpha=0.85, label=label)
    ax.set_xticks(x); ax.set_xticklabels([m for _,m in TABLE_METRICS], fontsize=10)
    ax.set_ylabel("Score"); ax.set_title("Weather — RL-MCF: 7 Metrics × 5 Forecasters", fontsize=13)
    ax.legend(fontsize=9); ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
    p = os.path.join(out_dir, "weather_barplot_5models.png")
...
def main():
    ...
    for label, rl_cfg, f_cfg in MODELS:
        results[label] = eval_one(label, rl_cfg, f_cfg, AE_CONFIG, device)
    print_table(results)
