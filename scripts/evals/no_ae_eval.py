...
from src.training.RL_trainers.RL_trainer_no_ae import RLNoAETrainer, build_temporal_mask, filter_batch
...
CONFIG_RL = "assets/configs/models/etth1_dataset/RL_ablations/noae.json"

CHECKPOINT_PATH = "assets/checkpoints/etth1_chpts/RL_ablations/noae/noae_agent_best.pt"
...
def forecast_monotonicity_with_context(
    ...
    for i in range(batch_size):
        ...
        if diff.ndim == 3:
            changed_idx = np.where(np.max(diff[0], axis=-1) > threshold)[0]
        else:
            changed_idx = np.where(diff[0] > threshold)[0]
...
@torch.no_grad()
def trainer_run_episode_noae(trainer, batch, filter_quantile=None):
    ...
    keep = filter_batch(y_hat, filter_quantile)
    if keep.sum() == 0:
        return None
    ...
    s = trainer.agent.build_state(x_ot, y_hat)

    mu, _ = trainer.agent.actor(s)
    action = mu.view(-1, trainer.cfg_f.seq_len, 1)
    ...
    x_cf = x_ot + temp_mask * (trainer.agent.eta * action)
    ...
    reward_dict = trainer.reward_fn(x_ot, x_cf, y_hat, y_cf, z_cf=None)
...
@torch.no_grad()
def evaluate_frozen_noae(trainer, evaluator, n_batches=20, include_dtw=False):
    trainer.agent.eval()
    ...
    for i, batch in enumerate(trainer.test_loader):
        ...
        ep = trainer_run_episode_noae(trainer, batch)
        if ep is None:
            continue
...
def save_examples_plot(examples, out_path, rho=0.10):
    ...
    plt.suptitle("Frozen checkpoint evaluation — No AE", fontsize=13)
    plt.tight_layout()
...
def main():
    ...
    trainer = RLNoAETrainer(cfg_f, cfg_rl, device)
    ...
    summary, examples = evaluate_frozen_noae(
        trainer=trainer,
        evaluator=evaluator,
        n_batches=20,
        include_dtw=False,
    ...
    for key in keys_to_show:
        if key in summary:
            print(f"{key:28s}: {summary[key]['mean']:.4f} ± {summary[key]['std']:.4f}")
