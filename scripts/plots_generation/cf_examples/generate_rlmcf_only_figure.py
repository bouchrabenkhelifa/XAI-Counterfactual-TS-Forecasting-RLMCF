#!/usr/bin/env python
"""
Generate RL-MCF Only Figure
============================
Show RL-MCF counterfactual generation on a single sample.

Usage:
    python scripts/generate_rlmcf_only_figure.py
"""
...
OUT_PATH    = "assets/figures/global_analysis/rlmcf_only_itransformer.png"
...
def main():
    ...
    print(f"Validity — RL-MCF: {vr_rm:.3f}")
    ...
    # RL-MCF: orange
    ax.plot(t_back, rm_full[:BH], color="#E65100", lw=2.0, ls="--")
    ax.plot(t_fore, rm_full[BH:], color="#E65100", lw=2.0, ls="-",
            label=f"$x_{{cf}}$ / $\hat{{y}}_{{cf}}$ RL-MCF  (VR={vr_rm:.2f})")
    ax.plot([BH-1, BH], rm_full[BH-1:BH+1], color="#E65100", lw=2.0, ls="-")
    ...
    y_min = 4
    y_max = max(orig_full.max(), rm_full.max(), b_denorm.max()) * 1.05
