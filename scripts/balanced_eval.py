...
from src.training.RL_strict_trainer import RLStrictTrainer
...
CHECKPOINT_PATH = "assets/checkpoints/RL_balanced/rl_balanced_agent_best.pt"
...
def save_examples_plot(examples, out_path, rho=0.10):
    ...
    plt.suptitle("Frozen checkpoint evaluation — RL Strict", fontsize=13)
    plt.tight_layout()
...
def main():
    ...
    # Construit le pipeline strict puis charge le checkpoint RL strict déjà appris
    ...
    # dict plat pour pretty_print_summary
    ...
    metrics_path = os.path.join(cfg_rl.results_dir_lp, "balanced_eval_metrics.json")
    with open(metrics_path, "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\n[Eval] Metrics saved -> {metrics_path}")

    fig_path = os.path.join(cfg_rl.figures_dir_lp, "balanced_eval_examples.png")
