...
cfg    = load_config("configs/models/itransformer/etth1_96_48_S.json")
...
for i, batch in enumerate(test_loader):
    ...
    mse = float(((y_orig - p_orig)**2).mean())
    axes[i].plot(t_past,   x_orig, color="steelblue", lw=1.5, label="Passé réel")
    axes[i].plot(t_future, y_orig, color="steelblue", lw=1.5, ls="-")
    axes[i].plot(t_future, p_orig, color="coral",     lw=1.5, ls="--", label="Prédit")
    axes[i].axvline(96, color="gray", ls="--", lw=1.2, label="now")
    axes[i].set_title(f"Sample {i+1} — MSE={mse:.2f}°C²  pred_len=48  use_norm=True", fontsize=11)
    axes[i].set_ylabel("OT (°C)")
    axes[i].legend(fontsize=9)
    axes[i].grid(alpha=0.3)

plt.suptitle("iTransformer S — pred_len=48  use_norm=True (°C)", fontsize=13)
plt.tight_layout()
plt.savefig("figures/forecaster/test_forecast_96_48_S.png", dpi=150)
plt.show()
print("Saved")