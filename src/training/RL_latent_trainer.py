...
def load_latent_plausibility(path, ae, device):
    ...
    plaus = LatentPlausibility(ae=ae,
                               alpha=ckpt["alpha"],
                               scale=ckpt["scale"])
    ...
    plaus.fitted_ = True
    print(f"[RL-Latent] LatentPlausibility loaded ✓")
    return plaus
...
def run_episode(batch, ae, forecaster, agent, reward_fn,
    ...
    x_ot, batch_x, batch_x_mark = x_ot[mask], batch_x[mask], batch_x_mark[mask]
    y_hat, z = y_hat[mask], z[mask]

    s              = agent.build_state(z, y_hat)
    a, log_prob, _ = agent.actor.sample(s)
    z_cf           = torch.clamp(z + agent.eta * a, -1.0, 1.0)

    # x_cf doit garder le gradient pour la plausibilité différentiable
    x_cf = ae.decode(z_cf)
    ...
    return {
        ...
        "y_hat": y_hat, "y_cf": y_cf,
        "z": z, "z_cf": z_cf.detach(), "n_valid": int(mask.sum()),
    }


class RLTrainerLatentPlaus:
    ...
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        print("\n[RL-Latent] Loading frozen models ...")
        ...
        # LatentPlausibility différentiable
        ...
        print(f"[RL-Latent] Objectif : réduire mean(forecast) de "
              f"{cfg_rl.rho*100:.0f}%")
        print(f"[RL-Latent] gamma = {cfg_rl.gamma} "
              f"(plausibilité différentiable)")
        ...
        print(f"[RL-Latent] Actor {params['actor']:,}  "
    ...
    def train(self):
        print(f"\n{'='*60}")
        print(f"RL-Latent Training | {self.cfg_rl.epochs} epochs | "
    ...
    def _save_checkpoint(self, tag):
        torch.save({
            ...
            "cfg_rl"           : vars(self.cfg_rl),
                        ...
                        f"rl_lp_agent_{tag}.pt"))

    def _save_history(self):
        path = os.path.join(self.cfg_rl.results_dir_lp,
                            "rl_lp_history.json")
        with open(path, "w") as f: json.dump(self.history, f, indent=2)
        print(f"[RL-Latent] History → {path}")
    ...
    def _plot_training(self):
        ...
        axes[0,1].set_title("Sub-Rewards"); axes[0,1].legend()
        axes[0,1].grid(alpha=0.3)

        axes[0,2].plot(self.history["delta_mean"],   color="coral",
                       lw=1.5, label="Δmean")
        axes[0,2].plot(self.history["success_rate"], color="green",
                       lw=1.5, ls="--", label="SR")
        axes[0,2].axhline(self.cfg_rl.rho, color="gray", ls=":", lw=1.2)
        axes[0,2].set_title("Δmean & SR"); axes[0,2].legend()
        axes[0,2].grid(alpha=0.3)

        axes[1,0].plot(self.history["loss_actor"],  color="coral",
                       lw=1.5, label="actor")
        axes[1,0].plot(self.history["loss_critic"], color="steelblue",
                       lw=1.5, label="critic")
        axes[1,0].set_title("Losses"); axes[1,0].legend()
        ...
        plt.suptitle(
            f"RL LatentPlausibility — objectif {self.cfg_rl.rho*100:.0f}%",
            fontsize=13)
        plt.tight_layout()
        path = os.path.join(self.cfg_rl.figures_dir_lp,
                            "rl_lp_training_curves.png")
        plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
        print(f"[RL-Latent] Curves → {path}")
    ...
    def _plot_cf_examples(self, examples):
        ...
        for i, ex in enumerate(examples):
            ...
            axes[i].set_title(
                f"Sample {i+1} — {reduction:+.1f}% {ok}", fontsize=11)
            axes[i].legend(fontsize=9); axes[i].grid(alpha=0.3)

        plt.suptitle("CF LatentPlausibility — ETTh1", fontsize=13)
        plt.tight_layout()
        path = os.path.join(self.cfg_rl.figures_dir_lp,
                            "rl_lp_cf_examples.png")
        plt.savefig(path, dpi=150, bbox_inches="tight"); plt.close()
        print(f"[RL-Latent] CF examples → {path}")