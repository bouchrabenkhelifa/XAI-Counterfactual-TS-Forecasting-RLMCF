...
def load_latent_plausibility(path, ae, device):
    ...
    plaus.fitted_ = True
    print("[RL-Latent] LatentPlausibility loaded ✓")
    return plaus
...
def run_episode(batch, ae, forecaster, agent, reward_fn, device, filter_quantile=0.75):
...
class RLTrainerLatentPlaus:
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        print(
            f"[RL-Latent] Objectif : réduire mean(forecast) de "
            f"{cfg_rl.rho * 100:.0f}%"
        )
        print(
            f"[RL-Latent] gamma = {cfg_rl.gamma} "
        ...
        print(f"[RL-Latent] Actor {params['actor']:,}  Critic {params['critic']:,}")
    ...
    def train(self):
        print(f"\n{'=' * 60}")
        print(
            f"RL-Latent Training | {self.cfg_rl.epochs} epochs | "
    ...
    def _save_checkpoint(self, tag):
        torch.save(
            ...
            os.path.join(self.cfg_rl.checkpoint_dir_lp, f"rl_lp_agent_{tag}.pt"),
        )

    def _save_history(self):
        path = os.path.join(self.cfg_rl.results_dir_lp, "rl_lp_history.json")
    ...
    def _plot_training(self):
        ...
        axes[1, 0].plot(
            self.history["loss_actor"],
            color="coral",
            lw=1.5,
            label="actor",
        )
        axes[1, 0].plot(
            self.history["loss_critic"],
            color="steelblue",
            lw=1.5,
            label="critic",
        )
        ...
        plt.tight_layout()
        path = os.path.join(self.cfg_rl.figures_dir_lp, "rl_lp_training_curves.png")
    ...
    def _plot_cf_examples(self, examples):
        ...
        for i, ex in enumerate(examples):
            ...
            reduction = (
                (y_hat.mean() - y_cf.mean()) / (abs(y_hat.mean()) + 1e-8) * 100
            )
        ...
        plt.tight_layout()
        path = os.path.join(self.cfg_rl.figures_dir_lp, "rl_lp_cf_examples.png")
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
