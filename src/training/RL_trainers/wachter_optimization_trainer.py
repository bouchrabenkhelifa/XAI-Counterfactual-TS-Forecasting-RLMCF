...
class OptimizationTrainer:
    ...
    def __init__(self, cfg_forecaster, cfg_ae, cfg_opt, device):
        ...
        print(
            f"[OptTrainer] Ready | rho={cfg_opt.rho} | "
    ...
    def _fit_detectors(self):
        ...
        self.if_model = IsolationForest(contamination=0.1, random_state=42).fit(X_train)
        self.lof_model = LocalOutlierFactor(novelty=True, contamination=0.1).fit(
            X_train
        )
        self.svm_model = OneClassSVM(kernel="rbf", nu=0.1).fit(X_train)
    ...
    def _plausibility_scores(self, x_cf_np: np.ndarray) -> dict:
        ...
        def norm(s):
            ...
            return float(torch.sigmoid(torch.tensor(s)).mean().item())

        return {
            "plausibility_if": norm(if_raw),
            "plausibility_lof": norm(lof_raw),
            "plausibility_ocsvm": norm(svm_raw),
            "plausibility": (norm(if_raw) + norm(lof_raw) + norm(svm_raw)) / 3,
        }
    ...
    def evaluate(self, n_batches: int = 20) -> dict:
        ...
        print(
            f"  n_steps={self.cfg_opt.n_steps} | "
        ...
        for i, batch in enumerate(self.test_loader):
            ...
            t0 = time.time()
            result = self.generator.generate(batch[0], batch[2], self.device)
            ...
            all_metrics["delta_mean"].append(float(result["delta_mean"].mean().item()))
            all_metrics["relative_red"].append(float(result["rel_red"].mean().item()))
            all_metrics["success"].append(float(result["success"].mean().item()))
        ...
        # Save results
        path = os.path.join(self.cfg_opt.results_dir, "wachter_latent_results.json")
    ...
    def _plot_cf_examples(self, examples):
        ...
        for i, ex in enumerate(examples):
            ...
            axes[i].set_title(f"Sample {i+1} — {ex['red']:+.1f}% {ok}", fontsize=11)
        ...
        plt.tight_layout()
        path = os.path.join(self.cfg_opt.figures_dir, "wachter_latent_cf_examples.png")
