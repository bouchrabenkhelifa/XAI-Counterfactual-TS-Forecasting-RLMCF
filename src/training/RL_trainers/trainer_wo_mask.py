...
=================================
Identique à trainer_last.py, sauf que le masque temporel est désactivé
quand use_mask=False dans le config.

Sans masque : x_cf = AE.decode(z_cf)  directement (toute la série est modifiée)
Avec masque : x_cf = x_ot + mask * (AE.decode(z_cf) - x_ot)  (seuls les derniers timesteps)
"""
...
def run_episode_train_wo_mask(
    batch, ae_arch, forecaster, agent, reward_fn, device,
    use_rl=True, mask_last_k=24, mask_ramp_k=8,
    filter_quantile=0.0, use_mask=True,
):
    ...
    if use_mask:
        ...
        # Sans masque : toute la série est remplacée par la reconstruction
        x_cf      = x_prop
        temp_mask = torch.ones_like(x_ot)
    ...
    return dict(
        log_prob=log_prob, reward=R, value=value, entropy=entropy,
        reward_dict=reward_dict,
        x_ot=x_ot.detach(), x_cf=x_cf.detach(),
        y_hat=y_hat.detach(), y_cf=y_cf.detach(),
        batch_x=batch_x.detach(), batch_x_mark=batch_x_mark.detach(),
        z=z.detach(), z_cf=z_cf.detach(),
        mask=temp_mask.detach(), n_valid=int(mask_keep.sum()),
    )


@torch.no_grad()
def run_episode_eval_wo_mask(
    batch, ae_arch, forecaster, agent, reward_fn, device,
    use_rl=True, mask_last_k=24, mask_ramp_k=8,
    filter_quantile=0.0, use_mask=True,
):
    ...
    return dict(
        ...
        y_hat=y_hat.detach(), y_cf=y_cf.detach(),
        batch_x=batch_x.detach(), batch_x_mark=batch_x_mark.detach(),
        reward_dict=reward_dict, n_valid=int(mask_keep.sum()),
    )


class RLNoMaskTrainer:
    """Trainer identique à RLMaskTrainer mais avec use_mask configurable."""
    ...
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        self.use_mask       = getattr(cfg_rl, "use_mask", True)

        for d in [cfg_rl.checkpoint_dir_lp, cfg_rl.figures_dir_lp, cfg_rl.results_dir_lp]:
            os.makedirs(d, exist_ok=True)

        print(f"\n[RL] use_mask={self.use_mask}  Loading frozen models …")
        ...
        _global_sigma = float(np.concatenate(_x_stds).mean())
        print(f"[RL] global_sigma={_global_sigma:.4f}")
        ...
        self.opt_critic = torch.optim.Adam(self.agent.critic.parameters(), lr=getattr(cfg_rl, "lr_critic", 1e-4))

        lr_decay   = getattr(cfg_rl, "lr_decay", 0.5)
        decay_step = max(1, cfg_rl.epochs // 2)
        self.sched_actor  = torch.optim.lr_scheduler.StepLR(self.opt_actor,  step_size=decay_step, gamma=lr_decay)
        self.sched_critic = torch.optim.lr_scheduler.StepLR(self.opt_critic, step_size=decay_step, gamma=lr_decay)

        self.history = {k: [] for k in [
            "loss_total", "loss_actor", "loss_critic",
            "reward_total", "r_validity", "r_validity_hard",
            "r_proximity", "r_smooth", "delta_mean",
            "success_rate", "advantage", "n_valid",
        ]}
        ...
        self.evaluator = CounterfactualEvaluator(
            ...
            fit_plausibility=(self.x_train_eval is not None),
        )
    ...
    def _compute_bounds_np(self, x_ot, y_hat):
        ...
        x2d, y2d  = x_ot[:, :, 0], y_hat[:, :, 0]

        if self.reward_fn.global_sigma is not None:
            sigma = np.full(x2d.shape[0], self.reward_fn.global_sigma)
        else:
    ...
    def train(self):
        print(f"\n{'='*60}")
        print(f"Training (use_mask={self.use_mask}) | {self.cfg_rl.epochs} epochs")
        print(f"{'='*60}")
        ...
        for epoch in range(1, self.cfg_rl.epochs + 1):
            ...
            for batch in self.train_loader:
                ep = run_episode_train_wo_mask(
                    batch=batch, ae_arch=self.ae_arch, forecaster=self.forecaster,
                    agent=self.agent, reward_fn=self.reward_fn, device=self.device,
                    use_rl=use_rl, mask_last_k=self.mask_last_k,
                    mask_ramp_k=self.mask_ramp_k, filter_quantile=self.filter_quantile,
                ...
                stats["r_smooth"].append(rs.get("smooth", 0.0))
    ...
    def evaluate(self, n_batches=20):
        print(f"\n[Eval] {n_batches} batches  use_mask={self.use_mask} …")
        self.agent.eval()
        use_rl = getattr(self.cfg_rl, "use_rl", True)
        ...
        alphas, betas = self._compute_bounds_np(all_x, all_y_hat)
        ...
        path = os.path.join(self.cfg_rl.results_dir_lp, f"{self.exp_name}_evaluation.json")
        ...
        self._plot_cf_examples(
            cf_examples,
            os.path.join(self.cfg_rl.figures_dir_lp, f"{self.exp_name}_cf_examples.png"),
        )
    ...
    def _plot_training(self):
        ...
        axes[0,0].plot(self.history["reward_total"], lw=1.5); axes[0,0].set_title("Total Reward"); axes[0,0].grid(alpha=0.3)
        ...
        axes[0,2].plot(self.history["success_rate"], lw=1.5); axes[0,2].set_title("Success Rate"); axes[0,2].grid(alpha=0.3)
        ...
        axes[1,1].plot(self.history["r_validity_hard"], lw=1.5, color="orange"); axes[1,1].set_title("Validity Hard"); axes[1,1].grid(alpha=0.3)
        axes[1,2].plot(self.history["n_valid"], lw=1.5); axes[1,2].set_title("Samples valides / epoch"); axes[1,2].grid(alpha=0.3)
        plt.suptitle(f"RL-CF wo_mask={not self.use_mask} — {self.exp_name}", fontsize=13)
        plt.tight_layout()
        path = os.path.join(self.cfg_rl.figures_dir_lp, f"{self.exp_name}_training_curves.png")
    ...
    def _plot_cf_examples(self, examples, out_path):
        ...
        for i, ex in enumerate(examples):
            ...
            alpha_np, beta_np  = alpha_t[0].numpy(), beta_t[0].numpy()
            t_all  = np.arange(len(x_ot) + len(y_hat))
            t_fore = np.arange(len(x_ot), len(x_ot) + len(y_hat))
            valid_ratio = float(((y_cf >= alpha_np) & (y_cf <= beta_np)).mean())
            axes[i].plot(t_all, np.concatenate([x_ot, y_hat]), lw=1.5, label="original + forecast")
            axes[i].plot(t_all, np.concatenate([x_cf, y_cf]),  lw=1.5, ls="--", label="CF + forecast_cf")
            axes[i].fill_between(t_fore, alpha_np, beta_np, alpha=0.20, color="green", label="α/β bounds")
            axes[i].axvline(len(x_ot), linestyle="--", lw=1.2, color="gray")
            axes[i].set_title(f"Sample {i+1} — validity_hard={valid_ratio:.2f}", fontsize=11)
            axes[i].legend(fontsize=9); axes[i].grid(alpha=0.3)
        plt.suptitle(f"CF examples — {self.exp_name} (use_mask={self.use_mask})", fontsize=13)
        plt.tight_layout()
