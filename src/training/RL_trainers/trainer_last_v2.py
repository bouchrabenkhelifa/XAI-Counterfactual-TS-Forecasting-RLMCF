...
class RLMaskTrainer:
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        self.forecaster = ForecasterWrapperV2(cfg_forecaster, device)
        ...
        # Compute global sigma from train set — used as fixed scale for bounds
        # This eliminates train/test mismatch when per-sample std varies
        _x_stds = []
        ...
        print(f"[RL] global_sigma={_global_sigma:.4f}  (fixed scale for bounds)")
    ...
    def _compute_bounds_np(self, x_ot, y_hat):
        """
        Numpy mirror exact de CFReward.compute_bounds().

        beta  = y_hat - gap          (toujours < y_hat)
        alpha = y_hat - gap - width
        gap   = rho * std(x),  width = fr * std(x)

        x_ot  : [K, BH, 1]
        y_hat : [K, H,  1]
        retourne alphas, betas — [K, H]
        """
        ...
        if self.reward_fn.global_sigma is not None:
            ...
            sigma = x2d.std(axis=1, ddof=1).clip(min=1e-4)  # [K]

        gap   = (rho * sigma)[:, np.newaxis]             # [K, 1]
        width = (fr  * sigma)[:, np.newaxis]             # [K, 1]
    ...
    def _plot_training(self):
        ...
        axes[0, 1].plot(self.history["r_smooth"],        lw=1.5, label="smooth")
