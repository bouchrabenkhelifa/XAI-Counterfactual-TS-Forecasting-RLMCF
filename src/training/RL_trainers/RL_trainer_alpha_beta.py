...
from src.models.RL.reward3 import CFReward
...
def run_episode_train(
    ...
    x_cf = (
        x_ot + temp_mask * (delta + alpha_hf * x_hf.detach())
        if use_rl
        else x_ot.clone()
    )
...
def run_episode_eval(
    ...
    x_cf = x_ot + temp_mask * (delta + alpha_hf * x_hf) if use_rl else x_ot.clone()
...
class RLMaskTrainer:
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        # ── reward α/β timestep-wise ──────────────────────────────────
        self.reward_fn = CFReward(  # ✅ rho_max supprimé
            shift=getattr(cfg_rl, "shift", -0.10),
        ...
        print(  # ✅ rho_max retiré du print
            f"[RL] rho={self.reward_fn.rho}  "
        ...
        if plaus_path and os.path.exists(plaus_path):
            ...
            print(f"[RL] Plausibility model loaded: {plaus_path}")
        else:
            print("[RL] No plausibility model")

        self.evaluator = CounterfactualEvaluator(
            plausibility_model=plausibility_model,
            rho=self.reward_fn.rho,
    ...
    def train(self):
        print(f"\n{'='*60}")
        print(
            f"Training | {self.cfg_rl.epochs} epochs | rho={self.reward_fn.rho}"
        )  # ✅ rho_max retiré
        print(f"{'='*60}")
    ...
    def _plot_training(self):
        ...
        axes[0, 2].axhline(0.5, linestyle="--", lw=1.2, color="red", label="seuil 50%")
        ...
        axes[1, 1].axhline(0.5, linestyle="--", lw=1.2, color="red", label="seuil 50%")
        axes[1, 1].set_title("Validity Hard (Validity Ratio papier)")
        axes[1, 1].legend()
        ...
        plt.suptitle(  # ✅ rho_max retiré
            f"RL α/β — {self.exp_name} | rho={self.reward_fn.rho}",
            fontsize=13,
        )
        plt.tight_layout()
        path = os.path.join(
    ...
    def _plot_cf_examples(self, examples, out_path):
        ...
        for i, ex in enumerate(examples):
            ...
            yh_t = torch.tensor(y_hat).unsqueeze(0)
            alpha_np, beta_np, _ = self.reward_fn.compute_bounds(
                yh_t
            )  # ✅ unpack target
            alpha_np = alpha_np[0].numpy()
            beta_np = beta_np[0].numpy()
            ...
            axes[i].fill_between(
                t_fore, alpha_np, beta_np, alpha=0.20, color="green", label="α/β bounds"
            )
