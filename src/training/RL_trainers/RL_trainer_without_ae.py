...
from src.models.RL_without_ae.agent import ActorCriticInputSpace
...
def run_episode(batch, forecaster, agent, reward_fn, device, filter_quantile=0.75):
    ...
    s = agent.build_state(x_ot, y_hat)
    a, log_prob, _ = agent.actor.sample(s)

    delta = agent.eta * a.unsqueeze(-1)  # (B, 96, 1)
    x_cf = x_ot + delta  # (B, 96, 1)
    ...
    reward_dict = reward_fn(x_ot, x_cf, y_hat, y_cf)
    R = reward_dict["total"]
    V = agent.evaluate(s)
...
class RLTrainerInputSpace:

    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        self.cfg_f, self.cfg_ae, self.cfg_rl = cfg_forecaster, cfg_ae, cfg_rl
        self.device = device

        for d in [
            cfg_rl.checkpoint_dir_input,
            cfg_rl.figures_dir_input,
            cfg_rl.results_dir_input,
        ]:
            os.makedirs(d, exist_ok=True)

        print("\n[RL-Input] Loading frozen models ...")
        ...
        self.plausibility = (
            ...
            else (obj.get("ensemble") or obj.get("model") or obj.get("plausibility"))
        )
        print(f"[RL-Input] Plausibility ✓  type={type(self.plausibility).__name__}")

        self.train_loader, self.test_loader, self.scaler = prepare_data(
        ...
        self.agent = ActorCriticInputSpace(
            seq_len=cfg_forecaster.seq_len,
            pred_len=cfg_forecaster.pred_len,
            eta=cfg_rl.eta_input,
            entropy_coef=cfg_rl.entropy_coef,
            direction=-1.0,
        ).to(device)

        params = self.agent.count_parameters()
        print(f"[RL-Input] Actor {params['actor']:,}  Critic {params['critic']:,}")
        print(f"[RL-Input] Objectif : réduire mean(forecast) de {cfg_rl.rho*100:.0f}%")
        print(f"[RL-Input] eta_input = {cfg_rl.eta_input}")

        self.opt_actor = torch.optim.Adam(
            self.agent.actor.parameters(), lr=cfg_rl.lr_actor
        )
        self.opt_critic = torch.optim.Adam(
            self.agent.critic.parameters(), lr=cfg_rl.lr_critic
        )
    ...
    def train(self):
        print(f"\n{'='*60}")
        print(f"RL-Input Training | {self.cfg_rl.epochs} epochs")
        print(f"{'='*60}")
        ...
        for epoch in range(1, self.cfg_rl.epochs + 1):
            ...
            for batch in self.train_loader:
                ep = run_episode(
                    batch, self.forecaster, self.agent, self.reward_fn, self.device
                )
                if ep is None:
                    continue
    ...
    def _save_checkpoint(self, tag):
        path = os.path.join(
            self.cfg_rl.checkpoint_dir_input, f"rl_input_agent_{tag}.pt"
        )
        torch.save(
            {
    ...
    def _save_history(self):
        path = os.path.join(self.cfg_rl.results_dir_input, "rl_input_history.json")
        with open(path, "w") as f:
            json.dump(self.history, f, indent=2)
        print(f"[RL-Input] History → {path}")
    ...
    def _plot_training(self):
        ...
        axes[0, 2].plot(
            self.history["delta_mean"], color="coral", lw=1.5, label="Δmean"
        )
        axes[0, 2].plot(
            self.history["success_rate"], color="green", lw=1.5, ls="--", label="SR"
        )
        ...
        axes[1, 0].plot(
            self.history["loss_actor"], color="coral", lw=1.5, label="actor"
        )
        axes[1, 0].plot(
            self.history["loss_critic"], color="steelblue", lw=1.5, label="critic"
        )
        ...
        plt.suptitle(
            f"RL Input Space — objectif {self.cfg_rl.rho*100:.0f}%  "
            f"eta={self.cfg_rl.eta_input}",
            fontsize=13,
        )
        plt.tight_layout()
        path = os.path.join(
            self.cfg_rl.figures_dir_input, "rl_input_training_curves.png"
        )
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"[RL-Input] Curves → {path}")
    ...
    def _plot_cf_examples(self, examples):
        ...
        for i, ex in enumerate(examples):
            ...
            axes[i].plot(
                t_all, full_orig, color="steelblue", lw=1.5, label="x + forecast(x)"
            )
            axes[i].plot(
                t_all,
                full_cf,
            ...
            axes[i].set_title(
                ...
                f"diff_passé={abs(x_cf-x_ot).mean():.3f}",
                fontsize=11,
            )
            axes[i].legend(fontsize=9)
            axes[i].grid(alpha=0.3)

        plt.suptitle("CF Input Space — ETTh1", fontsize=13)
        plt.tight_layout()
        path = os.path.join(self.cfg_rl.figures_dir_input, "rl_input_cf_examples.png")
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"[RL-Input] CF examples → {path}")
