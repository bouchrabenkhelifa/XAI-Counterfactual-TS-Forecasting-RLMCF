...
from src.models.RL.reward2 import CFReward
...
def prepare_rl_data(cfg_forecaster, cfg_ae, device):
    ...
    if "scaler_mean" in ckpt_ae and "scaler_std" in ckpt_ae:
        ...
        print(
            "[Warning] scaler not found in AE checkpoint — continuing with scaler=None"
        )
...
def run_episode_train(
    ...
    if use_rl:
        x_cf = x_ot + temp_mask * (delta + alpha_hf * x_hf.detach())
    else:
        x_cf = x_ot.clone()
...
def run_episode_eval(
    ...
    if use_rl:
        x_cf = x_ot + temp_mask * (delta + alpha_hf * x_hf)
    else:
        x_cf = x_ot.clone()
...
def forecast_monotonicity_with_context(
    x_ot, x_cf, x_full, x_mark, forecaster, threshold=5e-2
):
    ...
    for i in range(B):
        ...
        mask = (
            np.where(np.max(diff[0], axis=-1) > threshold)[0]
            if diff.ndim == 3
            else np.where(diff[0] > threshold)[0]
        )
        ...
        for t in mask:
            ...
            contributions.append(base_mean - abl_mean)
...
class RLMaskTrainer:
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        self.reward_fn = CFReward(
            rho=cfg_rl.rho,
            rho_max=getattr(cfg_rl, "rho_max", 2.0 * cfg_rl.rho),
            alpha=getattr(cfg_rl, "alpha", 2.0),
            beta=getattr(cfg_rl, "beta", 0.3),
            sigmoid_scale=getattr(cfg_rl, "sigmoid_scale", 5.0),
        ).to(device)

        print(
            f"[RL-Mask] rho          = [{cfg_rl.rho*100:.0f}%, "
            f"{getattr(cfg_rl, 'rho_max', 2*cfg_rl.rho)*100:.0f}%]"
        )
        print(f"[RL-Mask] alpha        = {getattr(cfg_rl, 'alpha', 2.0)}")
        print(f"[RL-Mask] beta         = {getattr(cfg_rl, 'beta', 0.3)}")
        print(f"[RL-Mask] sigmoid_scale= {getattr(cfg_rl, 'sigmoid_scale', 5.0)}")
        ...
        if plaus_path and os.path.exists(plaus_path):
            plausibility_model = load_plausibility_model(plaus_path)
            print(f"[RL-Mask] Plausibility model loaded: {plaus_path}")
        else:
            print("[RL-Mask] No plausibility model — evaluator running without it")
    ...
    def train(self):
        ...
        print(
            ...
            f"rho=[{self.cfg_rl.rho*100:.0f}%,"
            f"{getattr(self.cfg_rl, 'rho_max', 2*self.cfg_rl.rho)*100:.0f}%] | "
    ...
    def evaluate(self, n_batches=20):
        ...
        for i, batch in enumerate(self.test_loader):
            ...
            all_monot.append(
                forecast_monotonicity_with_context(
                    x_ot=ep["x_ot"],
                    x_cf=ep["x_cf"],
                    x_full=ep["batch_x"],
    ...
    def _plot_training(self):
        ...
        axes[0, 2].plot(self.history["success_rate"], lw=1.5, label="SR")
        ...
        axes[1, 1].axhline(
            self.cfg_rl.rho, linestyle=":", lw=1.2, color="green", label="rho"
        )
        axes[1, 1].axhline(
            getattr(self.cfg_rl, "rho_max", 2 * self.cfg_rl.rho),
            linestyle="--",
            lw=1.2,
            color="red",
            label="rho_max",
        )
        axes[1, 1].set_title("Δmean vs target zone")
        axes[1, 1].legend()
    ...
    def _plot_cf_examples(self, examples, out_path, title_prefix="CF"):
        ...
        rho_max = getattr(self.cfg_rl, "rho_max", 2 * self.cfg_rl.rho)
        ...
        for i, ex in enumerate(examples):
            ...
            in_zone = self.cfg_rl.rho * 100 <= reduction <= rho_max * 100
            ...
            axes[i].set_title(
                f"Sample {i+1} — reduction={reduction:+.1f}% {marker} "
                f"(target [{self.cfg_rl.rho*100:.0f}%, {rho_max*100:.0f}%])",
                fontsize=11,
            )
            axes[i].legend(fontsize=9)
            axes[i].grid(alpha=0.3)

        plt.suptitle(f"{title_prefix} RL-Mask — {self.exp_name}", fontsize=13)
        plt.tight_layout()
