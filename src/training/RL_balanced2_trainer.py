...
def load_latent_plausibility(path, ae, device):
    ...
    print("[RL-Balanced-v2] LatentPlausibility loaded ✓")
    return plaus
...
class RLBalancedV2Trainer:
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        print("\n[RL-Balanced-v2] Loading frozen models ...")
        ...
        self.reward_fn = CFReward(
            ...
            alpha=cfg_rl.alpha,
            beta=cfg_rl.beta,
            gamma=getattr(cfg_rl, "gamma", 0.30),
            kappa=getattr(cfg_rl, "kappa", 0.07),
        ).to(device)

        print(f"[RL-Balanced-v2] Objectif  : réduire mean(forecast) de {cfg_rl.rho * 100:.0f}%")
        print(f"[RL-Balanced-v2] gamma     = {getattr(cfg_rl, 'gamma', 0.30)}")
        print(f"[RL-Balanced-v2] kappa     = {getattr(cfg_rl, 'kappa', 0.07)}")
        print(f"[RL-Balanced-v2] alpha_hf  = {self.alpha_hf}")
        ...
        print(f"[RL-Balanced-v2] Actor {params['actor']:,}  Critic {params['critic']:,}")
    ...
    def train(self):
        print(f"\n{'=' * 60}")
        print(
            f"RL-Balanced-v2 Training | {self.cfg_rl.epochs} epochs | "
            f"rho={self.cfg_rl.rho} | eta={self.cfg_rl.eta} | "
            f"gamma={getattr(self.cfg_rl, 'gamma', 0.30)} | "
            f"kappa={getattr(self.cfg_rl, 'kappa', 0.07)} | "
    ...
    def _save_checkpoint(self, tag):
        torch.save({
            ...
            f"rl_balanced_v2_agent_{tag}.pt"
        ))

    def _save_history(self):
        path = os.path.join(
            self.cfg_rl.results_dir_lp,
            "rl_balanced_v2_history.json"
        )
        with open(path, "w") as f:
            json.dump(self.history, f, indent=2)
        print(f"[RL-Balanced-v2] History → {path}")
    ...
    def _plot_training(self):
        ...
        axes[0, 2].set_title("Balanced-v2 Plausibility Terms")
        axes[0, 2].legend()
        ...
        plt.suptitle(
            f"RL Balanced v2 — rho={self.cfg_rl.rho} | "
        ...
        path = os.path.join(
            ...
            "rl_balanced_v2_training_curves.png"
        )
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"[RL-Balanced-v2] Curves → {path}")
    ...
    def _plot_cf_examples(self, examples):
        ...
        for i, ex in enumerate(examples):
            ...
            axes[i].set_title(
                f"Sample {i+1} — {reduction:+.1f}% {ok} (balanced-v2)",
                fontsize=11
            )
            axes[i].legend(fontsize=9)
            axes[i].grid(alpha=0.3)

        plt.suptitle("CF RL-Balanced-v2 — ETTh1", fontsize=13)
        plt.tight_layout()
        path = os.path.join(
            self.cfg_rl.figures_dir_lp,
            "rl_balanced_v2_cf_examples.png"
        )
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"[RL-Balanced-v2] CF examples → {path}")
...
if __name__ == "__main__":
    ...
    CONFIG_RL = "assets/configs/models/etth1_dataset/RL/rl_balanced_v2.json"
    ...
    print("Pipeline : RL Balanced v2 (hybrid plausibility + latent drift)")

    trainer = RLBalancedV2Trainer(cfg_f, cfg_ae, cfg_rl, device)
    history = trainer.train()
    examples = trainer.evaluate(n_batches=20)

    print("\n✅ Done !")
    print(f"  Checkpoint → {cfg_rl.checkpoint_dir_lp}/rl_balanced_v2_agent_best.pt")
    print(f"  Figures    → {cfg_rl.figures_dir_lp}/")
    print(f"  Results    → {cfg_rl.results_dir_lp}/rl_balanced_v2_history.json")