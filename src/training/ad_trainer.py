...
from src.models.plausibility import (
    IForestPlausibility,
    LOFPlausibility,
    OCSVMPlausibility,
...
def load_series(cfg):
    ...
    print(f"[Data] {cfg.dataset_name} | train={len(ot_train_s)} val={len(ot_val_s)} test={len(ot_test_s)}")
...
def plot_sanity(scores_train, scores_val, dataset_name, figures_dir):
    ...
    ax.bar(x - width/2, [scores_train[k] for k in detectors], width, label="Train", color="steelblue")
    ax.bar(x + width/2, [scores_val[k]   for k in detectors], width, label="Val",   color="coral")
    ax.axhline(SANITY_THRESHOLD, color="red", linestyle="--", linewidth=1, label=f"Threshold ({SANITY_THRESHOLD})")
    ax.set_xticks(x)
...
def separation_test(ensemble, x_real, label="data"):
    ...
    print(f"  [{label}] score(real)={score_real:.4f}  score(noise)={score_noise:.4f}  gap={gap:.4f}  {status}")
    return score_real, score_noise, gap

if __name__ == "__main__":

    config_path = "configs/models/anomaly detector/plausibility.json"
    ...
    device    = "cpu"
    x_train_t = make_windows_tensor(ot_train_s, cfg.window, n_samples=512, device=device)
    ...
    sep_scores = {
        "gap_train": gap_tr, "gap_val": gap_vl,
        "score_real_train": s_real_tr, "score_noise_train": s_noise_tr,
        "score_real_val":   s_real_vl, "score_noise_val":   s_noise_vl,
    }
    ...
    print(f"\nSanity scores (threshold={SANITY_THRESHOLD}) :")
    print(f"  {'detector':<10}  {'train':>6}  {'val':>6}  status")
    print(f"  {'-'*38}")
    for k in ["if", "lof", "ocsvm", "ensemble"]:
        vt  = scores_train[k]
        vv  = scores_val[k]
        ok  = "✓" if vv > SANITY_THRESHOLD else "~"
        print(f"  {k:<10}  {vt:>6.4f}  {vv:>6.4f}  {ok}")

    print(f"\nSeparation test (gap = real - noise) :")
    print(f"  train : {gap_tr:.4f}  {'✓ good' if gap_tr > 0.15 else '✗ poor'}")
    print(f"  val   : {gap_vl:.4f}  {'✓ good' if gap_vl > 0.15 else '✗ poor'}")
    print(f"\n  → If gap is good, detector is ready for RL pipeline ✓")
