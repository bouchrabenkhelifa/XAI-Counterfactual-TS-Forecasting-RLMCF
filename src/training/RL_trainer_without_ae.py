...
def run_episode(batch, forecaster, agent, reward_fn,
                device, filter_quantile=0.75):
    ...
    return {
        "log_prob": log_prob, "reward": R, "value": V,
        "entropy": entropy,   "reward_dict": reward_dict,
        "x_ot": x_ot, "x_cf": x_cf, "y_hat": y_hat, "y_cf": y_cf,
...
class RLTrainerInputSpace:
    ...
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        for d in [cfg_rl.checkpoint_dir_input,
        ...
        with open(cfg_rl.plausibility_path, "rb") as f:
            obj = pickle.load(f)
        self.plausibility = obj if not isinstance(obj, dict) else (
        ...
        self.reward_fn = CFReward(
            ...
            rho=cfg_rl.rho, alpha=cfg_rl.alpha,
            beta=cfg_rl.beta, gamma=cfg_rl.gamma,
        ).to(device)
    ...
    def train(self):
        ...
        for epoch in range(1, self.cfg_rl.epochs + 1):
            ...
            for batch in self.train_loader:
                ep = run_episode(batch, self.forecaster,
    ...
    def _save_checkpoint(self, tag):
        path = os.path.join(
            self.cfg_rl.checkpoint_dir_input,
            f"rl_input_agent_{tag}.pt")
        torch.save({
    ...
    def _save_history(self):
        path = os.path.join(
            self.cfg_rl.results_dir_input,
            "rl_input_history.json")
    ...
    def _plot_training(self):
        ...
        plt.suptitle(f"RL Input Space — objectif {self.cfg_rl.rho*100:.0f}%  "
                     f"eta={self.cfg_rl.eta_input}", fontsize=13)
        plt.tight_layout()
        path = os.path.join(self.cfg_rl.figures_dir_input,
                            "rl_input_training_curves.png")
    ...
    def _plot_cf_examples(self, examples):
        ...
        for i, ex in enumerate(examples):
            ...
            axes[i].plot(t_all, full_orig, color="steelblue", lw=1.5,
                         label="x + forecast(x)")
            axes[i].plot(t_all, full_cf,   color="coral",     lw=1.5,
