...
from src.models.RL_nv.agent import ActorCritic
from src.models.RL_nv.reward import CFReward
...
class RLNVTrainer:
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        self.reward_fn = CFReward(
            ...
            tau=getattr(cfg_rl, "tau", 0.4),
        ).to(device)

        print(f"[RL-NV] Objectif  : réduire mean(forecast) de {cfg_rl.rho * 100:.0f}%")
        print(f"[RL-NV] alpha     = {cfg_rl.alpha}")
        print(f"[RL-NV] beta      = {cfg_rl.beta}")
        print(f"[RL-NV] gamma     = {getattr(cfg_rl, 'gamma', 0.8)}")
        print(f"[RL-NV] tau       = {getattr(cfg_rl, 'tau', 0.4)}")
        print(f"[RL-NV] alpha_hf  = {self.alpha_hf}")
        ...
        print(f"[RL-NV] Actor {params['actor']:,}  Critic {params['critic']:,}")
    ...
    def train(self):
        print(f"\n{'=' * 60}")
        print(
            f"RL-NV Training | {self.cfg_rl.epochs} epochs | "
            f"rho={self.cfg_rl.rho} | eta={self.cfg_rl.eta} | "
            f"gamma={getattr(self.cfg_rl, 'gamma', 0.8)} | "
            f"tau={getattr(self.cfg_rl, 'tau', 0.4)} | "
    ...
    def _plot_training(self):
        ...
        path = os.path.join(
            ...
            "rl_nv_training_curves.png"
        )
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"[RL-NV] Curves → {path}")
    ...
    def _plot_cf_examples(self, examples):
        ...
        plt.suptitle("CF RL-NV — ETTh1", fontsize=13)
        plt.tight_layout()
        path = os.path.join(
            self.cfg_rl.figures_dir_lp,
            "rl_nv_cf_examples.png"
        )
        plt.savefig(path, dpi=150, bbox_inches="tight")
        plt.close()
        print(f"[RL-NV] CF examples → {path}")
...
if __name__ == "__main__":
    ...
    CONFIG_RL = "assets/configs/models/RL/rl_nv.json"
    ...
    print("Pipeline : RL NV (reconstruction + temporal consistency)")

    trainer = RLNVTrainer(cfg_f, cfg_ae, cfg_rl, device)
    history = trainer.train()
    examples = trainer.evaluate(n_batches=20)

    print("\n✅ Done !")
    print(f"  Checkpoint → {cfg_rl.checkpoint_dir_lp}/rl_nv_agent_best.pt")
    print(f"  Figures    → {cfg_rl.figures_dir_lp}/")
    print(f"  Results    → {cfg_rl.results_dir_lp}/rl_nv_history.json")