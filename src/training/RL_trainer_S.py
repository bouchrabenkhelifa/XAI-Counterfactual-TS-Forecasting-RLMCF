...
def run_episode(batch, ae, forecaster, agent, reward_fn,
    ...
    return {
        ...
        "z": z, "z_cf": z_cf, "n_valid": int(mask.sum()),
    }
...
class RLTrainer:
    ...
    def __init__(self, cfg_forecaster, cfg_ae, cfg_rl, device):
        ...
        for d in [cfg_rl.checkpoint_dir, cfg_rl.figures_dir, cfg_rl.results_dir]:
        ...
        print(f"[RL] Plausibility ✓  type={type(self.plausibility).__name__}")
        ...
        print(f"[RL] Objectif : réduire mean(forecast) de {cfg_rl.rho*100:.0f}%")

        self.agent = ActorCritic(
            latent_dim=cfg_ae.latent_dim, pred_len=cfg_forecaster.pred_len,
