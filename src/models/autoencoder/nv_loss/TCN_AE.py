# src/models/tcn_ae.py
# ─────────────────────────────────────────────────────────────
# TCN Autoencoder — architecture originale INCHANGÉE
# Seul le trainer change (nouvelle loss dérivée)
#
...
class TCNEncoder(nn.Module):
    """Encodeur TCN — mean pooling original."""
    ...
    def forward(self, x):
        ...
        for b in self.blocks:
            out = b(out)
        return self.tanh(self.fc(out.mean(2)))   # mean pooling → (B, latent_dim)
...
class TCNAutoEncoder(nn.Module):
    """
    TCN Autoencoder — architecture originale.
    Entraîné avec loss dérivée (MSE + Δx + Δ²x).
    """
    ...
    def from_checkpoint(cls, ckpt_path, device=None):
        ...
        print(f"  loss_type  = {ckpt.get('loss_type', 'mse_only')}")
